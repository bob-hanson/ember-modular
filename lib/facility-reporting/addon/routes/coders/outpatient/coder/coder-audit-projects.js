...
export default FacilityEncountersBaseRoute.extend({
  ...
  model() {
    return this.modelFor('coders.outpatient.coder');
  },

  setupController(controller, currentCoder) {
    this._super(...arguments);
    set(controller, 'currentCoder', currentCoder);
    set(controller, 'currentProjects', this.modelFor('coders.outpatient'));
  },
  ...
  processTableOptions(table, options) {
    ...
    fd.append('coder_id', this.modelFor('coders.outpatient.coder').get('id'));
